...
for (i in 1:(length(loc_grp_v)))
{
  if (i != (length(loc_grp_v)))
  {
    ...
    for (y in 1:ddc_len)
    {
      ...
      if (length(ddc_clust) != 1)
      {
        ...
        for (d in 1: length(ddc_cluster))
        {
          ...
          dist_sum <- dist_sum + dy_dist$distance
        }
        
        new_dist_sum <- new_dist_sum + (dist_sum/ (length(ddc_cluster)))
      }
    ...
    new_dist_sum <- new_dist_sum / ddc_len
    ...
    print(paste0("The Ball Hall index is ", bh))
