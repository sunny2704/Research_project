...
bf_c <- as.data.frame(read.csv("E:/DDC Test/Meat_ClusterInfo.csv", header = F))
b_tr <- as.data.frame(read.csv("E:/DDC Test/Meat_TRAIN.csv", header = F))
b_te <- as.data.frame(read.csv("E:/DDC Test/Meat_TEST.csv", header = F))
...
for (i in 1:(length(loc_grp_v)))
{
  if (i != (length(loc_grp_v)))
  {
    ...
    part2 <-  (K-1) * WGSS
    answer <- part1 / part2
    #((N-K)/(K-1))* (BGSS/WGSS) (((N - K)/ (K-1)) * (BGSS/WGSS))
    ...
    print(paste0("The Calinski- Harabaz index is ", chi))
    BGSS <- 0.00
    WGSS <- 0.00
